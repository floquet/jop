...
if __name__ == "__main__":
    ...
    predator_vel = np.array( [ -1.0, -1.0, 0.0 ] )  # Predator initially stationary
    ...
    with open ( "simulation_output.txt", "w" ) as file_out:
        ...
        while True:
            ...
            d_prey_predator = np.linalg.norm( predator_loc - prey_loc )

            # Print diagnostics every 100 steps
            if k % 100 == 0:
                print(f"Step {k}: Time = {t:.2f} s, Separation = {d_prey_predator:.2f} m")

            # --- Detection Phase ---
            if not detected:
                if d_prey_predator < R_d:
            ...
            # --- Interception Phase ---
            if detected:
                direction = (prey_loc - predator_loc) / np.linalg.norm( prey_loc - predator_loc )
                predator_vel = direction * v_predator + prey_vel  # Add prey's velocity to predict interception point
                predator_loc = predator_loc + predator_vel * delta_t
            ...
            k += 1
            # Stop simulation if separation increases over successive steps
            if k > 1 and d_prey_predator > prev_d_prey_predator:
                print( f"Separation increased at step {k}. Stopping simulation." )
                break
            prev_d_prey_predator = d_prey_predator

            # Failsafe: Stop simulation after maximum iterations
            if k > 1000:
                print("Simulation terminated: Maximum iterations reached.")
                break
